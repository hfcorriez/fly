...
const { FN_RESERVE_REGEX, logger, md5File } = require('./utils')
...
class Loader {
  ...
  prepare (dir, prefix = '') {
    ...
    const envDefault = Loader.getEnvSettigns(path.join(dir, '.env'))
    if (envDefault) {
      info('load env file: .env')
      debug('env default:', envDefault)
      Object.assign(process.env, envDefault)
    }

    // Load env file
    const envCurrent = Loader.getEnvSettigns(path.join(dir, `.env.${this.options.env}`))
    if (envCurrent) {
      info(`load env file: .env.${this.options.env}`)
      debug('env current:', envCurrent)
      Object.assign(process.env, envCurrent)
    }

    // Load config
    debug('perpare to load:', dir, prefix)
  ...
  static getConfig (dir, env) {
    ...
    try {
      ...
      if (configEnvFile && fs.existsSync(configEnvFile)) {
        info('load fly config:', configEnvFile)
