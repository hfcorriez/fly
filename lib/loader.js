...
const FN_RESERVE_REGEX = new RegExp(`^(${FN_RESERVE_KEYS.join('|')})([a-zA-Z]+)`)
...
class Loader {
  ...
  list () {
    ...
    for (const prefix in this.options.mounts) {
      ...
      functions.push(...this.listDir(dir, { prefix }))
    }
    return functions
  }
  ...
  listDir (dir, options) {
    ...
    for (let file of fs.readdirSync(dir)) {
      ...
      if (isFile) {
        ...
        functions.push({ file: filePath, prefix, fn })
      } else if (
        stat.isDirectory() &&
   ...
   */
  load (file, options) {
    const { force, prefix = '', root = this.options.dir } = options || {}

    try {
      const { dir, ext, ignore } = this.options
