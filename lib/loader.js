...
const debug = logger('_loader', 'debug')
const error = logger('_loader', 'error')
const info = logger('_loader', 'info')
  ...
  prepare (dir, prefix = '') {
    ...
    if (this.options.hotreload && !this.isWatching) {
      ...
      chokidar.watch(dir, { ignoreInitial: true, ignored: /(^|[/\\])\../ }).on('all', (event, file) => {
        ...
        // When add file but not loaded success before
        if (event === 'change' && !this.files[file]) event = 'add'
        // Get filename
        ...
        switch (event) {
          case 'change':
            if (this.imports[file]) {
              ...
              if (this.reload(this.files[file].name)) {
                info('hot reload fn:', this.files[file].name)
              }
            } else if (filename.startsWith('fly.') && filename.endsWith('.yml')) {
              // this.loadConfig()
              // this.prepare(dir, prefix)
            } else if (file.endsWith('.js') && !file.endsWith('.test.js')) {
              this.load(file, { suppressError: true, force: true })
            }
            break
          case 'unlink':
            if (this.files[file]) {
              info('hot delete fn:', this.files[file].name, file)
              this.delete(this.files[file].name)
            }
            break
          case 'add':
            if (file.endsWith('.js') && !file.endsWith('.test.js')) {
              const newFn = this.load(file, { suppressError: true, force: true })
              if (newFn) {
                info('hot add fn:', newFn.name, file)
              }
            }
            break
        }
