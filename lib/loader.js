...
class Loader {
  ...
  prepare (dir, prefix = '') {
    Loader.loadEnv(dir)
    Loader.loadEnv(dir, this.options.env)
    Loader.loadEnv(dir, 'local')
   ...
   * @returns
   */
  static loadEnv (dir, type) {
    // Load env file
    const envFile = Loader.getEnvSettigns(path.join(dir, `.env${type ? `.${type}` : ''}`))
    if (!envFile) return

    info(`load env file: .env.${this.options.env}`)
    debug('env current:', envFile)
