...
const debug = logger('◻️loader', 'debug')
const error = logger('◻️loader', 'error')
const info = logger('◻️loader', 'info')
          ...
          case 'change':
            ...
            if (this.imports[file]) {
              ...
              if (!cache || cache.md5sum !== md5sum) {
                ...
                if (fn) {
                  info('hot reload fn:', fn.name, fn.path)
                }
              }
              ...
              if (fn) {
                info('hot load fn:', fn.name, fn.path)
              }
            }

            if (fn && !this.cache.get(fn.name)) {
              this.checkCache(fn)
            }
            break
          case 'unlink':
            ...
            if (deleteFn) {
              info('hot delete fn:', deleteFn.name, deleteFn.path)
