...
async function parseFormData (request, uploadConfig, rootTmpdir) {
  return new Promise((resolve, reject) => {
    const { allowTypes } = uploadConfig
    const maxSize = uploadConfig.maxSize || ONE_HUNDRED_MEGA_BYTE
    const result = {
      files: new Files(),
      fieldPairs: {}
    }
    ...
    function deleteTempDir (dir) {
      fs.readdir(dir, (err, files) => {
        if (err) {
          debug(`tmpDir not found, ${dir}, error: ${err.message}`)
        }
    ...
    function done (err) {
      if (err) {
        ...
        fileDoneCounter.tryResolve() // 调用 tryResolve(), 预防 文件先关闭 的情况
      }
    }
  }) // return new Promise()
}
...
/**
 *
 * @param {Files} files -- temp files uploaded
 */
...
class Files {
  ...
  toArray () {
    return Object.values(this).reduce((acc, v) => {
      if (Array.isArray(v)) {
        acc.push(...v)
      } else {
        acc.push(v)
