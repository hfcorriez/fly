...
module.exports = {
  ...
  async before (event) {
    ...
    if (this.fly) {
      ...
      if (event.args.hotreload) {
        Object.keys(this.fly.files).forEach(file => fs.watchFile(file, { interval: 500 }, _ => {
          console.debug(colors.yellow('HOT_RELOAD'), colors.grey(file))
          this.fly.reload(file)
        }))
      }
  ...
  async main (event, ctx) {
    if (!event.params.command) {
      ...
      let wait = await this.run(event.params, ctx)
      return { event, wait }
    }
  ...
  after (event) {
    if (event.wait) {
      ...
      console.log(utils.padding('  HOT RELOAD:', 12), colors.bold(event.event.args.hotreload || 'false'))
      return
    }
    process.exit(0)
  },
