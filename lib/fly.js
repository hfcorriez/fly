...
class Fly {
  ...
  bootstrap () {
    ...
    if (typeof this.config.project.hotreload !== 'boolean' && this.config.project.env === 'development') {
  ...
  getCtx (ctx, event, fn) {
    ...
    if (!ctx.fly) {
      ...
      ctx.fly = {
        ...
        call: this.buildCaller(({ fn, args: [ name, evt, context ] }) => {
          const n = typeof name === 'object' ? name : (this.exists(name) ? name : fn.prefix + name)
          return this.call(n, evt, Object.assign(ctx, {
    ...
    if (ctx._init !== fn.prefix) {
      ...
      // Assign ctx
      Object.assign(ctx, this.getCtxFor(fn.prefix))
    }
  ...
  getCtxFor (prefix) {
    ...
    if (!ctx) {
      ...
      Object.keys(this.functions).forEach(key => {
        ...
        ctx[key.substr(prefix.length)] = (evt, context) => this.call(key, evt, Object.assign(ctx, {
          originEventType: ctx.eventType,
          eventType: null,
          ...context
        }))
