...
class Fly extends EventEmitter {
  ...
  async call (name, event, initalContext) {
    ...
    if (ctx.eventType && fn.events[ctx.eventType]) {
      ...
      if (targetEvent.validate) {
        validChian.push([targetEvent.validate])
      }
  ...
  async invoke (main, event, ctx) {
    ...
    try {
      ...
      debug([
        utils.padding(trace.id, 26) + utils.padding(trace.type, '8') + utils.padding('← ' + (trace.eventType || '●'), '10'),
        utils.padding(trace.error || '+' + trace.spendTime + 'ms', 30),
        utils.padding(trace.eventId || '', 36),
      ].join(' | '))
    }
