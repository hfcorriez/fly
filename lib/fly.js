...
class Fly {
  ...
  async call (name, event, initialContext) {
    ...
    if (this.options.env === 'test' &&
      mockHandler &&
      Array.isArray(mockHandler.fnsToReload) &&
      mockHandler.fnsToReload.length) {
      debugMock('reload fly fns', mockHandler.fnsToReload)
      for (let fnPath of mockHandler.fnsToReload) {
        this.reload(require.resolve(fnPath))
      }
      mockHandler.fnsToReload = []
    }
    let targetEvent
    let fn =
      ...
      if (data.type === 'MOCK') {
        if (data.name === '_setMocks') {
          if (data.event) { // add lib mock
            const { flyFns, libFiles, reloads } = setMocks(data.event)
            this.fnNames = flyFns
            this.libFiles = libFiles
            this.fnsToReload = reloads
          } else {
