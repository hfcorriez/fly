...
class Fly {
  ...
  bootstrap () {
    ...
    Object.assign(this.config, Fly.GetConfig(this.config.dir, this.config.env))
    // Merge ignore
   ...
   */
  mount (dir, prefix) {
    if (this.mounted[dir]) return false

    this.add(dir, prefix)
    this.preload(dir)
    this.mounted[dir] = true
 ...
 */
  ...
  preload (dir) {
    ...
    if (config) {
      ...
      Object.keys(config.events || {}).forEach(event => {
        Object.keys(config.events[event]).forEach(name => {
          this.set(name, event, config.events[event][name])
        })
      })
    }
  ...
  list (type) {
    ...
    function filter (fns) {
      Object.keys(fns).forEach(name => {
        if (type && !fns[name].events[type]) return
        !functions.includes(fns[name]) && functions.push(fns[name])
      })
    }
  }
  ...
  async invoke (callee, fn, event, ctx) {
    ...
    ctx = this.getCtx(Object.assign(ctx, this.getFnCtx(fn, ctx)), fn, event)
  ...
  getCtx (ctx, event) {
    ...
    if (!ctx.call) {
      ctx.call = (name, evt, context) => {
        const n = this.exists(name) ? name : ctx.prefix + name
