...
class Fly {
  ...
  async call (name, event, initalContext) {
    ...
    for (let key of Object.keys(chain)) {
      ...
      try {
        event = await this.invoke(calleeFn, callee, event, ctx, invokeType, key)
        ctx.emit(key, event)
        if (event && event.$end) {
          event = event.$end
          break
    ...
    FNS_BEFORE.forEach(key => {
      ...
      if (key === 'props') {
        ...
        for (let beforeFnName of beforeFns) {
          ...
          const beforeChain = this.parse(beforeFn, { ...ctx, eventType: 'parse' })
   ...
   * @returns {*}
   */
  invoke (callee, fn, event, ctx, invokeType, key) {
