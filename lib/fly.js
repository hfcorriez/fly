...
class Fly {
  ...
  bootstrap () {
    ...
    debug('BOOTSTRAP', this.options.dir)
  ...
  load (file, options) {
    ...
    try {
      ...
      fn.components = { ...config.components, ...options.components, ...fn.components }
  ...
  add (dir, options) {
    ...
    const flyConfig = Fly.GetConfig(dir, this.options.env)
    if (
    // Fly config
      !flyConfig &&
        // Dir check
      ...
      debug('FLY_ERROR', 'can not load dir without fly.yml')
      return false
    }
    ...
    if (flyConfig) {
      if (flyConfig.ignore) {
        flyConfig.ignore = flyConfig.ignore.map(pattern => path.join(dir, pattern))
      }
      ...
      if (flyConfig.components) {
        ...
        flyConfig.components = { ...options.config.components, ...flyConfig.components }
      }
    }
    const config = this.configs[dir] = { ...options.config, ...flyConfig }
    ...
    fs.readdirSync(dir).forEach(file => {
      ...
      if (isFile) {
        this.load(filePath, { component: options.component })
      } else if (
        stat.isDirectory() &&
        ...
        this.add(filePath, { config, component: options.component })
      }
    })
  ...
  getContext (ctx, event) {
    ...
    if (!ctx.env) ctx.env = this.options.env
