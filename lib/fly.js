...
const { startMocks, stopMocks } = require('./mock/http')
...
class MockFly extends Fly {
  ...
  async call (name, event, initialContext) {
    ...
    // debugMock('mock handler', this.mockHandler)
  }
}
...
class MockHandler {
  ...
  constructor () {
    ...
    debug('will listen message event')
    process.on('message', data => {
      debugMock('test --> http:', data)
      if (data.type === 'MOCK') {
        if (data.name === '_startMocks') {
          const { flyFns, libFiles, reloadFns } = startMocks(data.event)
          this.fnNames = flyFns
          this.libFiles = libFiles
          this.reloadFns = reloadFns || []
          process.send({ type: 'MOCK', name: '_startMocks', event: 'OK' })
        } else if (data.name === '_stopMocks') {
          const { libFiles, reloadFns } = stopMocks(data.event)
          this.fnNames = new Set()
          this.libFiles = libFiles
          this.reloadFns = reloadFns
          process.send({ type: 'MOCK', name: '_stopMocks', event: 'OK' })
        ...
        } else if (data.name === '_exit') {
          process.send({ type: 'MOCK', name: '_exit', event: 'OK' })
          process.exit(data.event)
        } else {
  ...
  async call (fn, event, ctx) {
    ...
    debugMock('call mock : ', fn.name, JSON.stringify(event))
