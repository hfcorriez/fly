...
class Fly {
  ...
  load (file, force) {
    try {
      ...
      if (this.env !== 'test') {
        delete fn.mocks
      } else if (typeof fn.mocks === 'string') {
        fn.mocks = { '*': fn.mocks }
      }
  ...
  async call (name, event, initialContext) {
    ...
    targetEvent &&
      ...
      chain.push([targetEvent.validate, 'validate', 'event'])
    targetEvent &&
      targetEvent.before &&
      chain.push([targetEvent.before, 'before', 'event'])
    fn.validate && chain.push([fn.validate, 'validate'])
    fn.before && chain.push([fn.before, 'before'])
    fn.main && chain.push([fn.main, 'main'])
    fn.after && chain.push([fn.after, 'after'])
    targetEvent &&
      targetEvent.after &&
      chain.push([targetEvent.after, 'after', 'event'])

    while (chain.length) {
      let [callee, type] = chain.shift()
  ...
  async invoke (fn, event, ctx) {
    ...
    let ret
    if (this.env === 'test' && fn.mocks) {
      if (typeof event.mock === 'function') {
        return event.mock(event)
      }
      let e
      if (fn.mocks[event.mock]) {
        e = fn.mocks[event.mock]
      } else if (fn.mocks['*']) {
        e = fn.mocks['*']
      }
