...
const info = logger('◻︎fly', 'info')
const debug = logger('◻︎fly', 'debug')
const error = logger('◻︎fly', 'error')
const warn = logger('◻︎fly', 'warn')
...
class Fly {
  ...
  buildChain (fn, eventType) {
    ...
    const FNS_ALL = [...FNS_BEFORE, ...FNS_AFTER]
    ...
    FNS_BEFORE.forEach(key => buildChain(decorator, key))
    FNS_ALL.forEach(key => buildChain(fn, key))
    FNS_AFTER.forEach(key => buildChain(decorator, key))
    ...
    return Promise.all(
      this.loader.find(type).map(fn => {
        ...
        if (Object.keys(config).some(key => {
          ...
          info(`emit ${type} failed with fn config`)
          return false
        }
        this.call(fn.name, event, ctx)
      })
    )
  }
}
