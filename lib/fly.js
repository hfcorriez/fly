...
class Fly extends EventEmitter {
  ...
  async call (name, event, initalContext) {
    ...
    if (!fn) throw Fly.error('no function to call')
    ...
    try {
      while (chian.length) {
        ...
        try {
          ...
          if (type === 'validate') {
            if (!ret) throw Fly.error(`validate failed: ${fn.name}`)
          } else {
            event = ret
          }
        } catch (err) {
          callee = obj === 'event' ? (targetEvent.error || fn.error) : fn.error
          if (!callee) throw err
          event = err
          ctx = this.getContext(Object.assign(ctx, fn, { type }), event)
  ...
  getContext (initalContext, event) {
    ...
    if (!ctx.link) ctx.link = (name, link) => {
      if (link[0] === '.' || link[0] === '/') {
        ...
        throw Fly.error('internal link only support directory')
      }
    }
