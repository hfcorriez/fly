...
class Fly {
  ...
  resolve (link, config) {
    ...
    if (link[0] === '/' || link[0] === '.') {
      ...
      return {
        type: 'file',
        file: path.join(ROOT_DIR, './functions/' + link.substr(1))
      }
    } else if (/^[a-z0-9_-]$/.test(link)) {
      return {
        type: 'function',
        file: this.functions[link]
      }
  ...
  load (file, options) {
    ...
    try {
      ...
      fn.main = fn.main && fn.main.bind(fn)
   ...
   */
  preload () {
    console.log('functions', Object.keys(this.functions))
    Object.keys(this.functions).forEach(name => {
      const fn = this.functions[name]
      if (fn.extends) {
        const exName = fn.extends
        const exFn = this.functions[exName]
        if (!exFn) throw new FlyError(`${name} extends unknown "${exName}"`)
        Object.keys(exFn).forEach(key => {
          if (fn[key] || typeof exFn[key] !== 'function') return
          fn[key] = exFn[key].bind(fn)
        })
        this.extends[exName] = this.extends[exName] || []
        this.extends[exName].includes(name) || this.extends[exName].push(name)
      }
    })
  }
