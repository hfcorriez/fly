...
class Fly extends EventEmitter {
  ...
  get (name, ctx) {
    ...
    let tryIds = []
    if (!service) {
      ctx && tryIds.push(`${ctx.dir}:${name}`)
      tryIds = [`${this.options.dir}:${name}`]
    } else {
      tryIds = [`${this.options.dir}:${service}@${name}`]
      ctx && tryIds.push(`${ctx.file}:${service}@${name}`)
    }

    debug('get from names', tryIds.join('|'))

    while (tryIds.length) {
      let tryId = tryIds.shift()
      if (this.functions[tryId]) return this.functions[tryId]
    }
  ...
  add (file, options) {
    try {
      ...
      if (fn.links) {
        this.links(fn.links, { fn })
      }
      this.files[fn.file] = fn
      debug('function load:', fn.file)
      return fn
    } catch (err) {
  ...
  load (dir, options) {
    ...
    files.forEach(name => {
      ...
      if (stat.isFile() && file.endsWith('.js')) {
         ...
         */
        if (options.fn && options.link) {
          this.functions[options.fn.file + ':' + options.link + '@' + fn.name] = fn
        } else if (options.dir && options.link) {
          this.functions[options.dir + ':' + options.link + '@' + fn.name] = fn
        }
        this.functions[fn.dir + ':' + fn.name] = fn
      } else if (stat.isDirectory() && !this.options.ignoreDirNames.includes(name)) {
        this.link(name, { path: file, dir })
      }
    })

    // Process links
    if (config && config.links) {
      this.links(config.links, { dir, config: config && config[options.name + '@'] })
    }
  }
  ...
  link (name, options) {
    ...
    if (options.path[0] === '.' || options.path[0] === '/') {
       ...
       */
      this.load(
        options.path[0] === '/' ? options.path : path.join(options.dir || this.options.dir, options.path),
  ...
  getContext (initalContext, event) {
    ...
    if (!ctx.call) {
      ctx.call = (name, evt, context) => {
        return this.call(name, evt, Object.assign(ctx, {
          type: '',
        }, context || {}))
      }
    }
