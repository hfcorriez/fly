...
const info = logger('fly', 'fly', 'debug')
const error = logger('fly', 'fly', 'error')
const warn = logger('fly', 'fly', 'warn')
...
class Fly {
  ...
  async call (name, event, initalContext) {
    ...
    for (let key of Object.keys(chain)) {
      ...
      try {
        ...
        if (!catchFn) {
          error(`call failed: ${fn.name}`, err.trace)
   ...
   */
  parseChain (fn, ctx) {
    info('parse chain:', fn.name, ctx.eventType)
    const chain = {}
    ...
    FNS_BEFORE.forEach(key => {
      ...
      if (key === 'props') {
        ...
        for (let preloadFnName of preloadFns) {
          const preloadFn = this.loader.get(preloadFnName)
          if (!preloadFn) {
  ...
  async invoke (callee, fn, event, ctx, invokeType) {
    info('invoke:', `${fn.name}:${invokeType}`)
    ...
    try {
      ...
      const traceInfo = [
        `(${trace.eventType || '-'}) ${trace.name}:${invokeType}`,
        trace.spendTime + 'ms',
        trace.error ? `[${trace.error.name}] ${trace.error.stack}` : 'OK',
        trace.eventId.split('-').pop() || '-'
      ].join(' ')
      trace.error ? error('invoke fail:', traceInfo) : info('invoke ok:', traceInfo)
    }
  ...
  broadcast (type, event, ctx) {
    if (!type) {
      warn(`no type to broadcast`)
      return
    }
    if (!SYSTEM_EVENTS.includes(type)) {
      error('event type is do not support broadcast')
      return
    }
