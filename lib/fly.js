...
class Fly extends EventEmitter {
  ...
  load (dir, service, options) {
    ...
    if (!options.subdir) {
      if (this.functions[serviceKey]) {
        console.warn('service already exists:', serviceKey, dir)
        return
      }
      this.functions[serviceKey] = {}
    }
    ...
    files.forEach(fileName => {
      ...
      if (stat.isFile() && file.endsWith('.js')) {
        ...
        if (this.functions[serviceKey][fn.name]) {
          debug(`duplicate name for ${fn.name}`, file)
          return
        }

        this.functions[serviceKey][fn.name] = fn
  ...
  loadFunction (file, service, { config, root }) {
    try {
      ...
      // Process api
      fn.service = service
      fn.name = fn.name || this.buildFunctionName(file, root)
