...
class Fly {
  ...
  async call (name, event, initalContext) {
    ...
    while (chian.length) {
      let [catchFn, type, obj] = chian.shift()
      // Restore ctx
      ctx = this.getContext(Object.assign(ctx, fn, { type }), event)
      try {
        let ret = await this.invoke(catchFn, event, ctx)
        if (type === 'validate') {
        ...
        catchFn = obj === 'event' ? (targetEvent.catch || fn.catch) : fn.catch
        if (!catchFn) throw err
        event = err
        ctx = this.getContext(Object.assign(ctx, fn, { type }), event)
        return await this.invoke(catchFn, event, ctx)
      }
    }
    return event
  }
