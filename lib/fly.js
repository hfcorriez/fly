...
class Fly {
  constructor (config) {
    ...
    this.compiedCache = new Cache({ dir: this.config.project })
    this.cache = this.compiedCache.all()
  ...
  get (name) {
    ...
    if (this.lazyloadFunctions[name]) {
      const { file, options } = this.lazyloadFunctions[name]
 ...
 */
   ...
   */
  load (file, options) {
    const { force, prefix = '', root = this.config.project.dir, init = false } = options || {}
    ...
    try {
      ...
      if (!ext.some(e => file.endsWith('.' + e))) {
        debug('ignore by ext:', relativePath)
        return
      } else if (micromatch.any(relativePath, ignore, { basename: true })) {
        debug('ignore by rule:', relativePath)
        return
      }

      if (init && prefix !== '$' && !force && this.config.project.lazyload) {
        const fnName = prefix + file.split('/').pop().split('.js').shift()
        const isLazyload = Object.keys(this.config).some(event => {
          return Object.keys(this.config[event]).some(name => name === fnName)
        })
        if (!isLazyload) {
          debug('add to lazyload:', relativePath)
          delete options.init
          this.lazyloadFunctions[fnName] = {
            file, options
          }
          return
        }
      ...
      // Remove from lazyload functions when it loaded
      if (this.lazyloadFunctions[fn.name]) {
        delete this.lazyloadFunctions[fn.name]
      }
  ...
  add (dir, { prefix, root }) {
    debug('add dir', dir)
    const { dirs } = this.config.project
    if (!dir) {
    ...
    fs.readdirSync(dir).forEach(file => {
      ...
      const isInDir = prefix !== '$' && dirs && dirs.every(d => !relativePath.startsWith(d))
      if (isInDir) {
        debug('ignore by project.dirs:', filePath)
        return
      }

      if (isFile) {
        this.load(filePath, { prefix, root, init: true })
      } else if (
        stat.isDirectory() &&
        ...
        if (!isAdd) {
          debug('ignore dir by project.ignore:', filePath)
          return
        }
  ...
  async invoke (callee, fn, event, ctx, invokeType) {
    ...
    try {
      ...
      trace.error ? warn('invoke fail:', traceInfo) : debug('invoke ok:', traceInfo)
    }
    ...
    if (trace.error) {
      ...
      if (ctx.leftRetries <= 0) {
        ...
        warn('retry failed after', ctx.retry, 'times')
  ...
  getCtx (ctx, event, fn) {
    ...
    Object.keys(this.lazyloadFunctions).forEach(key => {
      if (fn.prefix && !key.startsWith(fn.prefix)) return
