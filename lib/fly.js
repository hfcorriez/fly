...
class Fly {
    ...
    try {
      ...
      if (fn.extends) {
        ...
        if (!resolved || !resolved.file) {
          throw Fly.Error(`extends "${fn.extends}" error: ${file}`)
        }
      ...
      if (config.functions && config.functions[fn.name]) {
        Fly.MergeConfigs(fn, config.functions[fn.name])
      }
  ...
  add (dir, options) {
    if (!dir) {
      throw Fly.Error('no dir argument passed')
    }
    ...
    if (!dirStat || !dirStat.isDirectory()) {
      throw Fly.Error('not dir or dir not exists: ' + dir)
    }
  ...
  static Error (message, code, type) {
    ...
    err.type = type ? type.toUpperCase() : 'FlyError'
    err.code = code || 1
    return err
  }
  ...
  static MergeConfig (config, serviceConfigItem, name) {
    ...
    console.log('config', config, serviceConfigItem, name)
