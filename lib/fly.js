 ...
 */
  ...
  getCtxGroup (prefix, ctx) {
    ...
    if (!ctxGroup) {
      ...
      Object.keys(this.functions).forEach(key => {
        ...
        const fly = this
        ctxGroup[key.substr(prefix.length)] = function (evt, context) {
          return fly.call(key, evt, Object.assign(ctx, {
            originEventType: ctx.eventType,
            eventType: null,
            ...context,
            $app: this.$app
  ...
  setApp (app) {
    this.config.project.app = app || 'all'
  }

  getApp () {
    return this.config.project.app
  }
