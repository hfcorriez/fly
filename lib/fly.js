...
class Fly {
  ...
  load (file, options) {
    ...
    try {
      ...
      if (fn.default && fn.default.toString().startsWith('class ')) {
        const Klass = fn.default
        fn = new Klass()
      }
      ...
      const name = (fn.name || camelcase(path.basename(file, '.js')))
  ...
  extend (name, fromName) {
    ...
    Object.keys(fromFn).concat(getInstanceMethodNames(fromFn)).forEach(key => {
  ...
  async call (name, event, initalContext) {
    ...
    if (!Array.isArray(fn.chain)) {
      ...
      const FNS_AFTER = eventTypeCap ? FN_EXEC_AFTER.concat(FN_EXEC_AFTER.map(f => f + eventTypeCap)) : FN_EXEC_AFTER

      FNS_BEFORE.forEach(key => {
        if (key === 'props' && fn.props) {
          fn.chain.push([(event) => Validator.validateEvent(event, fn.props), fn, key])
        } else if (fn[key]) {
          fn.chain.push([fn[key], fn, key])
        }
      })
      // [AFTER] function exec
      FNS_AFTER.forEach(key => fn[key] && fn.chain.push([fn[key], fn, key]))
    }
...
function camelcase (origin) {
  ...
  for (let i in origin) {
    if (i === '0') {
      ...
      name += origin[i].toLowerCase()
    } else {
      if (origin[i] === '-') {
        upper = true
      } else if (upper) {
        upper = false
        name += origin[i].toUpperCase()
      } else {
        name += origin[i]
      }
    }
