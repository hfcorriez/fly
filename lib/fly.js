...
class Fly {
  ...
  add (file, options) {
    try {
      ...
      if (fn.functions) {
        Object.keys(fn.functions).forEach(name => {
          const resolveObj = this.resolve(fn.functions[name], { dir: fn.dir })
          if (!resolveObj || !resolveObj.file) throw Fly.Error(`cannot resolve function "${name}" in ${fn.file}`)
          fn.functions[name] = resolveObj.file
        })
      }
      ...
      Fly.OutputWarning('LOAD_FAILED', err.message, file)
      return false
    }
  }
  ...
  load (dir, options) {
    ...
    files.forEach(name => {
      ...
      if (isFile && micromatch.any(serviceAbsFile, ignorePatterns)) {
        debug('IGNORE by ignore rules', serviceAbsFile)
      } else if (isFile && !micromatch.any(serviceAbsFile, filePatterns)) {
        debug('IGNORE by files rules', serviceAbsFile)
      } else if (isFile) {
  ...
  getContext (ctx, event) {
    ...
    if (ctx.functions && Object.keys(ctx.functions).length) {
      Object.keys(ctx.functions).forEach(name => {
  ...
  static MergeConfigs (fn, dirConfig) {
    ...
    fn.functions = Fly.MergeConfig(fn.functions, dirConfig.functions, fn.name)
    if (fn.events) {
