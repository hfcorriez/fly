 ...
 */
  ...
  getCtx (ctx, event) {
    ...
    if (!ctx._init) {
      ...
      ctx.debug = this.buildLogger('debug', ctx)
      ctx.info = this.buildLogger('info', ctx)
      ctx.warn = this.buildLogger('warn', ctx)
      ctx.error = this.buildLogger('error', ctx)
      ctx.fatal = this.buildLogger('fatal', ctx)
