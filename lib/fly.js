...
class Fly {
   ...
   * @param {Object} flyConfig
   */
  static MergeConfig (config, flyConfig, fn) {
  ...
  static MergeConfigs (fn, flyConfig) {
    flyConfig = flyConfig || {}
    fn.config = Fly.MergeConfig(fn.config, flyConfig.config, fn)
    fn.links = Fly.MergeConfig(fn.links, flyConfig.links, fn)
    if (fn.events) {
      Object.keys(fn.events).forEach(type => {
        if (!flyConfig.events || !flyConfig.events[type]) return
        Fly.MergeConfig(fn.events[type], flyConfig.events[type], fn)
      })
    } else {
      fn.events = {}
    }
