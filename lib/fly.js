...
const DEFAULT_OPTIONS = {
  ...
  supportEvents: ['startup', 'shutdown', 'error'],
  dir: process.cwd(),
  force: false,
  hotreload: false
}
...
class Fly {
  ...
  add (dir, config) {
    ...
    // Overwrite again
    config = this.configs[dir] = { ...config, ...flyConfig }
  ...
  broadcast (type, event, ctx) {
    ...
    if (!this.config.supportEvents.includes(type)) { throw new FlyError('event type is do not support broadcast') }
