...
class Fly {
  ...
  resolve (link, options) {
    ...
    if (link[0] === '/' || link[0] === '.') {
      ...
      return {
        type: 'file',
        file: path.join(ROOT_DIR, './base/' + link.substr(1))
      }
  ...
  load (file, force) {
    try {
      ...
      const absFile = path.join(path.relative(config.flyDir || root, dir), path.basename(file))
      const filePatterns = config.files || this.options.files
      const ignorePatterns = config.ignore || this.options.ignore

      if (micromatch.any(absFile, ignorePatterns)) {
        debug('IGNORE_RULE', absFile)
        return
      } else if (!micromatch.any(absFile, filePatterns)) {
        debug('IGNORE_FILES', absFile)
        return
      }
  ...
  add (dir, options) {
    ...
    if (this.dirs[dir]) {
      ...
      config = this.dirs[dir] = Object.assign(options.config || {}, flyConfig || {})
      if (flyConfig) {
        config.flyDir = dir
      }
    }
    ...
    if (ctx.imports && Object.keys(ctx.imports).length) {
      Object.keys(ctx.imports).forEach(name => {
        ctx[name] = (evt, context) => {
          return this.call(this.get(name), evt, Object.assign(ctx, { type: '', eventType: null }, context || {}))
        }
      })
    }
