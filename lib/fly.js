...
class Fly {
  ...
  load (file, options) {
    ...
    try {
      ...
      if (fileObj.toString().startsWith('class ')) {
        fn = new fileObj()
      } else if (typeof fileObj === 'function') {
        fn = { main: fileObj }
      } else {
        fn = { ...require(file) }
      }
      if ((typeof fn.main !== 'function' && !fn.extends) || (typeof fn.main === 'function' && fn.extends && typeof fn.extends !== 'string')) {
  ...
  extend (name, fromName) {
    ...
    Object.keys(fromFn).concat(Utils.getInstanceMethodNames(fromFn)).forEach(key => {
      if (fn[key] || typeof fromFn[key] !== 'function' || key === 'constructor') return
  ...
  async call (name, event, initalContext) {
    ...
    const eventType = (ctx.eventType && Utils.ucfirst(ctx.eventType)) || 'DEFAULT'
    debug('CALL', fn.name, ctx.eventType)

    fn.chains = fn.chains || {}
    if (!Array.isArray(fn.chains[eventType])) {
      fn.chains[eventType] = []
      const FNS_BEFORE = eventType ? FN_EXEC_BEFORE.map(f => f + eventType).concat(FN_EXEC_BEFORE) : FN_EXEC_BEFORE
      const FNS_AFTER = eventType ? FN_EXEC_AFTER.concat(FN_EXEC_AFTER.map(f => f + eventType)) : FN_EXEC_AFTER

      FNS_BEFORE.forEach(key => {
        if (key === 'props' && fn.props) {
          fn.chains[eventType].push([(event) => Validator.validateEvent(event, fn.props), fn, key])
        } else if (fn[key]) {
          fn.chains[eventType].push([fn[key], fn, key])
        }
      })
      // [AFTER] function exec
      FNS_AFTER.forEach(key => fn[key] && fn.chains[eventType].push([fn[key], fn, key]))
    }
    const chain = fn.chains[eventType]

    for (let i = 0; i < chain.length; i++) {
      // Restore ctx
      const [callee, calleeFn, invokeType] = chain[i]
      try {
        event = await this.invoke(callee, calleeFn, event, Object.assign(ctx, { invokeType }))
      } catch (err) {
        const callee = fn['catch' + eventType ]|| fn.catch
        if (!callee) {
          this.error(err)
          throw err
        }
