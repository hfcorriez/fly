...
const debugMock = require('debug')('TEST:MOCK:FLY')
...
class Fly {
  constructor (options) {
    debugMock('fly run with raw options: ', options)
    this.clients = {}
    this.dirs = {}
    this.files = {}
    this.functions = {}
    ...
    debug('fly run with options', this.options)
    this.bootstrap()
  }
...
class MockFly extends Fly {
  ...
  async call (name, event, initialContext) {
    if (!name) throw Fly.Error('no name to call')
    if (this.options.env === 'test' && this.mockHandler.reloadFns.length) {
      debugMock('reload fly fns', this.mockHandler.reloadFns)
      for (let fnPath of this.mockHandler.reloadFns) {
        this.reload(require.resolve(fnPath))
      }
...
class MockHandler {
  ...
  constructor () {
    ...
    process.on('message', data => {
      ...
      if (data.type === 'mock') {
        ...
        } else if (data.fn === '_exitMock') {
          process.send({ type: 'mock', fn: '_exitMock', event: 'OK' })
          process.exit()
  ...
  _startMock ({ file }) {
    if (!file) {
      throw new Error(`*.mock.js file not found`)
    }
    debugMock('will load mock file ', file)
    this.mocks = (file && require('rewire')(file)) || {}
    ...
    for (let _path of libFiles) {
      debug('reload lib file:', require.resolve(_path))
