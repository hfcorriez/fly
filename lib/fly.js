 ...
 */
  ...
  extend (name, fromName) {
    ...
    Utils.allKeys(fromFn).forEach(key => {
      ...
      FNS_BEFORE.forEach(key => {
        if (key === 'props' && fn.props) {
          ...
          for (let hookFnName of hookFns) {
            ...
            fn.chains[eventType].push([(event, ctx) => this.call(hookFnName, event, ctx), fn, key])
          }
        }
      })
