 ...
 */
  ...
  getCtxCache (prefix) {
    ...
    if (!ctxCache) {
      ...
      * Process imports
      */
      Object.keys(this.config.import).forEach(key => {
        debug('import', key, this.config.import[key])
        const filePath = path.join(this.config.dir, this.config.import[key])
