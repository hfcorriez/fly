 ...
 */
  ...
  getCtx (ctx, event, fn) {
    ...
    if (ctx._init !== fn.prefix) {
      // Check ctx is new
      const isFresh = ctx.hasOwnProperty('_init')
      ...
      // Assign cache to context to make it reuseable
      Object.assign(ctx, this.getCtxCache(fn.prefix, ctx))
    }
   ...
   * @param {String} prefix
   */
  getCtxCache (prefix, ctx) {
    let ctxCache = this.ctxGroups[prefix]
    if (!ctxCache) {
      ...
      Object.keys(this.functions).forEach(key => {
        ...
        ctxCache[key.substr(prefix.length)] = (evt, context) => this.call(key, evt, Object.assign(ctx, {
          originEventType: ctx.eventType,
          eventType: null,
          ...context
        }))
      ...
      */
      ...
      this.ctxGroups[prefix] = ctxCache
    }
