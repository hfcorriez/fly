...
class Fly {
  ...
  bootstrap () {
    ...
    debug('bootstrap:', JSON.stringify({ dir: this.config.project.dir, name: this.config.project.name, env: this.config.project.env }))
  ...
  getCtxCache (prefix) {
    ...
    if (!ctxCache) {
      ...
      Object.keys(this.config).forEach(key => {
        if (!ctxCache[key]) ctxCache[key] = this.config[key]
      })
...
class MockFly extends Fly {
  constructor (options) {
    debug('mock fly run with options: ', options)
    super(options)
    this.mockHandler = MockHandler.create()
  }
  async call (name, event, initialContext) {
    if (!name) throw Fly.Error('no name to call')
    if (this.config.project.env === 'test' && this.mockHandler.reloadFns.length) {
      debugMock('reload fly fns', this.mockHandler.reloadFns)
      for (let fnName of this.mockHandler.reloadFns) {
        debugMock('reload fn: ', fnName, this.reload(fnName))
      }
      this.mockHandler.reloadFns = []
    }
    let fn = typeof name.main === 'function' ? name : this.get(name, initialContext)
    if (!fn) throw Fly.Error(`no function to call: ${name}`)

    try {
      const data = await this.mockHandler.call(fn, event)
      if (data === MOCK_FN_NOT_MATCH) {
        debugMock(`no match mock: ${fn.name}`)
        debugMock('fly call ', name.name || name)
        return super.call(name, event, initialContext)
      }
      return data
    } catch (err) {
      throw new Error(`Fatal Error call ${fn.name}, ${err}`)
    }
  }
}

const debugMock = require('debug')('TEST:MOCK_FLY')
var _mockHandler
class MockHandler {
  static create () {
    if (!_mockHandler) {
  ...
  constructor () {
    ...
    process.on('message', data => {
      debugMock('test --> http:', data)
      if (data.type === 'mock') {
        if (data.fn === '_startMock') {
          const { flyFns, reloadFns } = this._startMock(data.event)
          this.fnNames = flyFns || new Set()
          this.reloadFns = reloadFns || []
          process.send({ type: 'mock', fn: '_startMock', event: 'OK' })
        } else if (data.fn === '_stopMock') {
          const { reloadFns } = this._stopMock(data.event)
          this.fnNames = new Set()
          this.reloadFns = reloadFns
          process.send({ type: 'mock', fn: '_stopMock', event: 'OK' })
        } else if (data.fn === '_rewireMock') {
          this._rewireMock(data.event)
          process.send({ type: 'mock', fn: '_rewireMock', event: 'OK' })
        } else {
          debugMock('Unexpected event', data)
        }
      }
    })
  }
  ...
  _startMock ({ file }) {
    debugMock('will load mock file ', file)
    if (!fs.existsSync(file)) {
      this.mocks = {}
    } else {
      this.mocks = require('rewire')(file) || {}
    }
    this.mocks.fly = this.mocks.fly || {}
    const flyFns = new Set(Object.keys(this.mocks.fly || {}))

    this.mocks.lib = this.mocks.lib || {}
    const libFiles = Object.keys(this.mocks.lib)
    for (let _path of libFiles) {
      debugMock('rewire lib file -:', require.resolve(_path))
      debugMock('lib props:', this.mocks.lib[_path])
      delete require.cache[require.resolve(_path)]
      const _module = require(_path)
      for (let _prop of Object.keys(this.mocks.lib[_path])) {
        debugMock('reset lib props', _prop)
        _module[_prop] = this.mocks.lib[_path][_prop]
      }
    }

    const reloadFns = this.mocks.reload || []
    return { flyFns, reloadFns }
  }
  ...
  _rewireMock ({ rewiredObj }) {
    debugMock('_rewireMock', this.mocks)
    for (let _prop of Object.keys(rewiredObj)) {
      this.mocks.__set__(_prop, rewiredObj[_prop]) // 修改 mock fly 的私有变量来控制 mock 返回值
    }
  }

  async call (fn, event, ctx) {
    if (!this.fnNames.has(fn.name)) {
      return MOCK_FN_NOT_MATCH
    }
    debugMock('call mock', fn.name, 'with event', JSON.stringify(event))
    return this.mocks.fly[fn.name](event, ctx)
  }
}

debugMock('use fly: ', process.env.NODE_ENV === 'test' ? 'MockFly' : 'Fly')

module.exports = process.env.NODE_ENV === 'test' ? MockFly : Fly
