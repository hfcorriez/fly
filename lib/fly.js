...
const FN_DEFINE_REGEX = new RegExp(
  `^(${FN_RESERVE_KEYS.joiconst yaml = require('js-yaml')
  const path = require('path')
  const fs = require('fs')
  const uuidv4 = require('uuid/v4')
  const Client = require('./client')
  ...
  class Fly {
    ...
    async invoke (fn, event, ctx) {
      if (this.env === 'test' && this.mailBox) {
        const data = await this.mailBox.invoke(fn, event, ctx)
        if (data.name === fn.name) {
          return data.event
        }
      }
  ...
  class MailBox {
    ...
    async invoke (fn, event, ctx) {
      return new Promise((resolve, reject) => {
        process.send({ name: fn.name, event })
        this.addHandler(data => resolve(data))
      })
    }
    addHandler (handler) {
...
class Fly {
  ...
  async call (name, event, initalContext) {
    ...
    let targetEvent
    let fn =
      typeof name.main === 'function' ? name : this.get(name, initalContext)
    if (!fn) throw Fly.Error(`no function to call: ${name}`)
