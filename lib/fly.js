...
class Fly {
  ...
  reload (file) {
    ...
    let reloadFiles = [file]
    if (this.extends[file]) reloadFiles = reloadFiles.concat(this.extends[file])

    reloadFiles.forEach(filePath => {
      delete this.files[filePath]
      delete require.cache[filePath]
      this.load(filePath, options)
      debug('FN_RELOAD', filePath)
    })
    return true
  }
