...
class Fly extends EventEmitter {
  ...
  get (name, ctx) {
    ...
    console.log('get', name, link, Object.keys(this.functions))
  ...
  async call (name, event, initalContext) {
    ...
    if (!fn) throw this.error('no function to call')
    ...
    targetEvent && targetEvent.validate && chian.push([targetEvent.validate, 'validate'])
    targetEvent && targetEvent.before && chian.push([targetEvent.before, 'before'])
    fn.validate && validChian.push([fn.validate, 'validate'])
    ...
    try {
      while (chian.length) {
        ...
        if (type === 'validate') {
          if (!ret) throw this.error(`validate failed: ${fn.name}`)
        } else {
          event = ret
        }
      }
