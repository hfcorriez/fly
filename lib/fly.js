...
class Fly extends EventEmitter {
  ...
  load (dir, service, options) {
    ...
    // Process links
    if (config && config.links) this.loadLinks(config.links, { dir })
  }
  ...
  loadFunction (file, service, { config, root }) {
    try {
      ...
      if (fn.links) {
        this.loadLinks(fn.links, { dir: fn.dir })
      }
  ...
  async invoke (main, event, ctx) {
    ...
    try {
      if (!ctx.trace.type) ctx.trace.type = 'internal'
      ...
      debug([
        ...
        '|', utils.padding(ctx.trace.id, 26),
        '|', utils.padding(ctx.trace.type || 'internal', '10'),
        '|', ctx.trace.error || '+' + ctx.trace.spendTime + 'ms'
      ].join(' '))
    }
  ...
  getContext (initalContext, event) {
    ...
    if (!ctx.add) ctx.add = (link, service) => {
      if (link[0] === '.' || link[0] === '/') {
        this.loadLinks({ [service]: link })
      } else {
