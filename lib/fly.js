 ...
 */
  ...
  extend (name, options) {
    ...
    const fn = this.get(name)
    if (!fn) return false
    from = from || fn.extends

    // Delete when force
    if (force && this.extends[fn.extends]) {
      this.extends[fn.extend].splice(this.extends[fn.extends].indexOf(name), 1)
    }
  ...
  async invoke (callee, fn, event, ctx, invokeType) {
    debug(`invoke [${fn.name}:${invokeType}]`, JSON.stringify(event))
    ctx = this.getCtx(ctx, event)
  ...
  getCtx (ctx, event) {
    ...
    if (!ctx._init) {
      ctx._init = true
      debug('init ctx for:', ctx.eventType, ctx.eventId)
      ...
      ctx.call = async (name, evt, context) => {
        ...
        let result, err
        try {
          result = await this.call(n, evt, Object.assign(ctx, {
      ...
      ctx.super = this.buildCaller(({ fn, caller, event }) => {
      ...
      ctx.list = this.list.bind(this)
      ctx.get = this.get.bind(this)
      ctx.broadcast = this.broadcast.bind(this)
      ctx.debug = this.buildCaller(({ fn, args }) => Fly.Logger(this.config.project.name, 'debug', fn.name)(...args))
      ctx.info = this.buildCaller(({ fn, args }) => Fly.Logger(this.config.project.name, 'info', fn.name)(...args))
      ctx.warn = this.buildCaller(({ fn, args }) => Fly.Logger(this.config.project.name, 'warn', fn.name)(...args))
      ctx.error = this.buildCaller(({ fn, args }) => Fly.Logger(this.config.project.name, 'error', fn.name)(...args))
      ctx.fatal = this.buildCaller(({ fn, args }) => Fly.Logger(this.config.project.name, 'fatal', fn.name)(...args))

      Object.keys(this.functions).forEach(key => {
        ctx[key] = (evt, context) => this.call(key, evt, Object.assign(ctx, {
          originEventType: ctx.eventType,
          eventType: null,
          ...context
        }))
