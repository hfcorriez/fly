...
class Fly {
  ...
  buildChain (fn, eventType) {
    ...
    const buildChain = (fn, key, value) => {
      ...
      if (key === 'props') {
        ...
        for (let chainFnName of chainFns) {
          ...
          if (!chainFn) {
            throw new FlyError(`no functon ${chainFnName} found`)
          }
          debug('get chain:', chainFnName)
          // parse before fn chain
          const beforeChain = this.buildChain(chainFn, eventType)
          Object.keys(beforeChain).forEach(k => {
            const n = `${chainFnName}:${k}`
            if (chain[n]) return
            chain[n] = beforeChain[k]
    ...
    FNS_AFTER.forEach(key => {
      ...
      console.log('after', key, decorator)
