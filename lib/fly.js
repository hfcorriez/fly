...
class Fly {
  ...
  load (file, options) {
    ...
    try {
      ...
      const basename = (fn.name || path.basename(file, '.js'))
      ...
      fn.component = options.component
  ...
  async call (name, event, initalContext) {
    ...
    let eventFn
    let fn = typeof name.main === 'function' ? name : this.get(name)
    ...
    fn.components && Object.keys(fn.components).forEach(cname => {
      FNS_BEFORE.forEach(key => {
        ...
        if (this.functions[fnName]) {
          chian.push([(event, ctx) => this.call(fnName, event, ctx), this.functions[fnName], 'component'])
        }
      })
    })
    ...
    FNS_BEFORE.forEach(key => {
      if (key === 'props' && fn.props) {
        chian.push([(event) => Validator.validateEvent(event, fn.props), fn, key])
      } else if (fn[key]) {
        chian.push([fn[key], fn, key])
      }
    })

    // [AFTER] function exec
    FNS_AFTER.forEach(key => fn[key] && chian.push([fn[key], fn, key]))

    // [AFTER] Component function exec
    fn.components && Object.keys(fn.components).forEach(cname => {
      FNS_AFTER.forEach(key => this.functions[`${cname}@${key}`] && chian.push([(event, ctx) => this.call(`${cname}@${key}`, event, ctx), this.functions[`${cname}@${key}`], 'component']))
    })

    while (chian.length) {
      // Restore ctx
      const [callee, calleeFn, invokeType] = chian.shift()
      try {
        const ret = await this.invoke(callee, calleeFn, event, Object.assign(ctx, { invokeType }))
        ...
        return this.invoke(callee, calleeFn, err, ctx)
      }
    }
  ...
  async invoke (callee, fn, event, ctx) {
    ...
    ctx = this.getCtx(Object.assign(ctx, this.getFnCtx(fn)), event)
  ...
  getCtx (ctx, event) {
    ...
    if (!ctx.call) {
      ctx.call = (name, evt, context) => {
        name = ctx.component ? `${ctx.component}@${name}` : name
    ...
    if (!ctx.loadFunctions || ctx.loadForComponent !== ctx.component) {
      ...
      Object.keys(this.functions).forEach(name => {
        ...
        // Clear all component functions
          const [ component, fn ] = name.split('@')
          if (ctx.component === component && !FN_DEFINE_REGEX.test(fn)) {
            ctx[fn] = (evt, context) => this.call(name, evt, Object.assign(ctx, { invokeType: '', eventType: null }, context || {}))
            ctx.loadFunctions.push(fn)
          }
        }
      })
    }
    ctx.loadForComponent = ctx.component
