 ...
 */
  ...
  load (file, options) {
    ...
    try {
      ...
      debug('LOAD_FN', fn.name)
      return fn
    } catch (err) {
      warn('LOAD_FN_ERROR', err)
  ...
  async call (name, event, initalContext) {
    ...
    if (ctx.eventType && fn.events && fn.events[ctx.eventType]) {
      eventFn = fn.events[ctx.eventType]
    }

    const chain = fn.chain || this.parseChain(fn, ctx)
    const keys = Object.keys(chain)
    info('call', `${fn.name}[${ctx.eventType || '-'}]`, keys.join(' > '))

    for (let key of Object.keys(chain)) {
      const [callee, calleeFn, invokeType] = chain[key]
      try {
  ...
  parseChain (fn, ctx) {
    ...
    FNS_BEFORE.forEach(key => {
      ...
      if (key === 'props') {
        chain[name] = chain[name] || [(event) => Validator.validateEvent(event, fn.props), fn, key]
      } else if (key === 'preload') {
        ...
        chain[name] = chain[name] || [value, fn, key]
    ...
    FNS_AFTER.forEach(key => {
      if (!fn[key]) return
      const name = `${fn.name}:${key}`
      chain[name] = chain[name] || [fn[key], fn, key]
      debug('add chain', name, 'to', fn.name)
    })
  ...
  getCtx (ctx, event) {
    ...
    if (ctx.for !== ctx.name) {
      ctx.for = ctx.name
      debug('CTX_INIT', ctx.name, 'logger and functions')
