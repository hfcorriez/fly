...
const os = require('os')
const debug = require('debug')('fly/cli/dis')
...
const QI_HOME = path.join(os.homedir(), '.fly')
...
class Discover extends Http {
  constructor (options) {
    ...
    if (options.links && options.links.length) {
      this.QI = require(path.join(this.constructor.getServiceLibPath(), 'lib/fly'))
      if (!this.QI) {
        console.error('"fly-node" not found in global path, ignore links')
      } else {
        options.links.forEach(dir => {
          let fly = new this.QI({ dir }, this)
          this.linkedServices[fly.name] = fly
        })
      }
    }
  ...
  getServiceConfig () {
    if (this.options.service) {
      let functions = {}
      Object.keys(this.fly.functions).map(name => {
        let fnConfig = this.fly.functions[name]
      ...
      return {
        ...
        value: JSON.stringify({
          gateways: this.fly.gateways,
          functions,
          settings: this.fly.settings
        })
      }
    } else if (this.options.gateway) {
      return {
  ...
  static getServiceLibPath () {
    try {
      let commandPath = require('child_process').execSync('which fly-node').toString().trim()
      let flyPath = fs.realpathSync(commandPath)
      return path.join(flyPath, '../../')
    } catch (err) {
      return false
    }
  }
