...
const debug = logger('▶cache', 'debug')
// const error = logger('▶loader', 'error')
class Cache {
  constructor (options) {
    this.options = options
    const projectCachePath = path.join(this.options.dir, '.fly')
    if (!fs.existsSync(projectCachePath)) {
      fs.mkdirSync(projectCachePath, { recursive: true })
    }
    this.projectCacheFilePath = `${projectCachePath}/functions.cache.js`
    debug('cache path:', this.projectCacheFilePath)
    try {
   ...
   */
  save () {
    debug('save cache to:', this.projectCacheFilePath)
