...
module.exports = {
  ...
  validateMapper: {
    ...
    numeric: input => typeof input === 'number' || Validator.isNumeric(input),
    port: 'isPort',
    url: 'isURL',
    ...
    enum: (input, definition) => definition.enum && definition.enum.includes(input),
  ...
  validateEvent (event, props, name) {
    ...
    Object.keys(props).forEach(key => {
      ...
      if (!valid) {
        errors.push({
          ...
          type,
          input: event[key],
          message: message || 'validate failed'
        })
    ...
    if (errors.length) {
      throw new FlyValidateError(`${name} validate failed ${errors.map(error => error.path).join(', ')}`, { errors })
    }
  ...
  validateOne (input, definition) {
    ...
    if (typeof definition === 'function') {
      try {
        ...
        return { valid: ret === true, value: input, type: 'function' }
      } catch (e) {
        return { valid: false, type: 'function', message: e.message }
      }
    } else if (definition instanceof RegExp) {
      const ret = typeof inptu === 'string' ? definition.test(input) : false
      return { valid: ret, value: ret ? input : undefined, type: 'pattern' }
    }
