...
module.exports = {
  ...
  validateEvent (event, props, name) {
    ...
    if (errors.length) {
      throw new FlyValidateError(errors.map(e => `${e.message}`).join(', '), { errors })
    }
  ...
  validateOne (input, definition, name) {
    ...
    if (typeof input === 'undefined') {
      if (!definition.required || definition.optional) {
        ...
        return { errors: [{ path, input, type: 'undefined', message: definition.message || `${path} invalid ${type}` }] }
      }
    ...
    if (!valid) {
      return { errors: [{ path, input, type, message: definition.message || `${path} invalid ${type}` }] }
    }
