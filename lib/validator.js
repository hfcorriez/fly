...
module.exports = {
  ...
  validateEvent (event, props, name) {
    ...
    if (errors.length) {
      throw new FlyValidateError(`validate failed: ${errors.map(e => `${e.message}`).join(', ')}`, { errors })
    }
  ...
  validateOne (input, definition, name) {
    ...
    if (typeof definition === 'function' && this.systemTypes[definition.name]) {
      ...
      try {
        ...
        return { errors: [{ path, input, type: 'custom', message: `validate failed: ${e.message}` }] }
      }
    } else if (definition instanceof RegExp) {
      definition = { type: 'pattern', pattern: definition }
    }
    ...
    if (!type && definition.pattern) {
      type = 'pattern'
    } else if (TYPE_NAMES.includes(type)) {
    ...
    if (typeof input === 'undefined') {
      if (!definition.required || definition.optional) {
        ...
        return { errors: [{ path, input, type: 'undefined', message: definition.message || `${path} invalid with ${type}` }] }
      }
    } else if (this.types[type]) {
      if (typeof input === 'string' && definition.trim) input = input.trim()
      const map = this.types[type]
    ...
    if (!valid) {
      return { errors: [{ path, input, type, message: definition.message || `${path} invalid with ${type}` }] }
    }
