...
exports.camelcase = function camelcase (origin) {
  let camel = ''
  let upper = false
  for (let i in origin) {
    if (i === '0') {
      ...
      camel += origin[i].toLowerCase()
    } else {
      if (origin[i] === '-') {
        upper = true
      } else if (upper) {
        upper = false
        camel += origin[i].toUpperCase()
      } else {
        camel += origin[i]
      }
    }
...
const EXCLUDE_KEYS = Object.getOwnPropertyNames(Object.getPrototypeOf({}))

exports.allKeys = function allkeys (fn) {
  let proto = fn
  let keys = []
  do {
    for (let key of Object.getOwnPropertyNames(proto)) {
      if (!keys.includes(key) && !EXCLUDE_KEYS.includes(key)) {
        keys.push(key)
      }
    }
