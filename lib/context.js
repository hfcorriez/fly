...
class Context {
  ...
  static from (data, event, fn, fly) {
    ...
    return new Context(data, event, fn, fly, context).toProxy()
  }
  ...
  init () {
    ...
    if (!data.eventId) {
      console.log('build', data.eventId, this.fn.name, this.parentContext && this.parentContext.fn.name)
    ...
    if (!data.fly) {
      data.fly = {
        ...
        call: this.buildCaller(({ fn, args: [ name, evt, context, isolation ] }) => {
          ...
          return this.fly.call(fnName, evt, this.expand(context))
