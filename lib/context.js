...
class Context {
  ...
  load (key) {
    ...
    try {
      if (isFile) {
        ...
        if (filePath.endsWith('.js') || filePath.endsWith('.json')) {
          if (this.fly.options.env === 'development' && !Context.WATCH_FILES[filePath]) {
            ...
            fs.watchFile(filePath, _ => {
              warn('hotreload:', key, filePath)
              // delete require cache
              delete require.cache[filePath]
              // delete instance
              delete this.instance[filePath]
            })
          }
          loaded = require(filePath)
          // Create a global instance
          if (key[0] === '#') {
            if (this.instance[filePath]) {
              loaded = this.instance[filePath]
            } else if (typeof loaded === 'function') {
              // Check if is a class or file
              if (/^\s*class\s+/.test(loaded.toString())) {
                // eslint-disable-next-line new-cap
                loaded = this.instance[filePath] = new loaded()
              } else {
                loaded = this.instance[filePath] = loaded()
              }
            } else {
      ...
      warn(`failed to load context[${key}]`, err)
      // Ignore
