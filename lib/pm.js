...
class PM {
  ...
  start (app, options) {
    ...
    const json = [{
      ...
      env: Object.assign({
        ...
        ...app.cluster ? { CLUSTER: app.cluster } : null,
      }, process.env, app.env || {}),
      instances: app.instance || '1',
  ...
  stop (name) {
    return new Promise((resolve, reject) => {
      this.pm2.connect(err => {
        ...
        this.pm2.delete(`/^${this.options.name}:${suffix}$/`, _ => resolve())
      })
    })
  }
  ...
  async list (name) {
    ...
    list = await new Promise((resolve, reject) => {
      this.pm2.connect(err => {
        if (err) return reject(err)
        this.pm2.list((err, list) => err ? reject(err) : resolve(list))
      })
    })
  ...
  async status (name) {
    ...
    list.forEach(item => {
      table.push([
        (item.status === 'online' ? colors.green('◉') : colors.red('◉')) + ` ${item.name}`,
        item.pid,
        item.port,
        item.memory,
        item.cpu,
  ...
  restart (name) {
    return new Promise((resolve, reject) => {
      this.pm2.connect(err => {
        ...
        const suffix = name !== 'all' ? '(?:' + name + ')' : '.*?'
        this.pm2.restart(`/^${this.options.name}:${suffix}$/`, err => err ? reject(err) : resolve())
      })
    })
  }
  ...
  reload (name) {
    return new Promise((resolve, reject) => {
      this.pm2.connect(err => {
        ...
        const suffix = name !== 'all' ? '(?:' + name + ')' : '.*?'
        this.pm2.reload(`/^${this.options.name}:${suffix}$/`, err => err ? reject(err) : resolve())
      })
    })
  }
