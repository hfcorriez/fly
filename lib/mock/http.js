...
const REWIRE_OK_FROM_HTTP = 'rewire http done'
const EXIT_OK_HTTP = 'http receive exit'
  ...
  http.on('message', data => {
    ...
    if (data.type === 'MOCK') {
      if (data.name === '_startMocks') {
        ...
        return eventEmitter.emit(REWIRE_OK_FROM_HTTP, data)
      } else if (data.name === '_exit') {
        return eventEmitter.emit(EXIT_OK_HTTP, data)
      }
      // send to mock
      if (mock) {
...
exports.stopFlyHttpServer = async function () {
  return new Promise(resolve => {
    eventEmitter.once(EXIT_OK_HTTP, resolve)
    http && http.send({ type: 'MOCK', name: '_exit', event: 0 })
  })
}
  ...
  mock.on('message', data => {
    ...
    if (data.type === 'MOCK') {
      ...
      if (data.name === '_status') {
        ...
        eventEmitter.emit(REWIRE_OK_FROM_MOCK, data)
...
exports.rewireMockServer = async function rewireMockServer (rewiredObj, file) {
  file = _getMockFile(file)
  debug(rewiredObj, file)
  mock.send({ type: 'MOCK', name: '_rewire', event: { file, rewiredObj } })
  http.send({ type: 'MOCK', name: '_rewire', event: { file, rewiredObj } })
  return Promise.all([
    new Promise(resolve => {
      eventEmitter.once(REWIRE_OK_FROM_MOCK, resolve)
    }),
    new Promise(resolve => {
      eventEmitter.once(REWIRE_OK_FROM_HTTP, resolve)
    })
  ])
}
